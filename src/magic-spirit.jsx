...
export default function MagicSpiritGame() {
  ...
  const verifyStateSync = useCallback(() => {
    ...
    // ヘルパー: 配列比較（長さと各要素のuniqueIdを比較）
    const compareArrays = (name, engineArr, useStateArr) => {
      if (!engineArr && !useStateArr) return;
      if (!engineArr || !useStateArr) {
        mismatches.push(`${name}: engine=${engineArr?.length ?? 'null'}, useState=${useStateArr?.length ?? 'null'}`);
        return;
      }
      if (engineArr.length !== useStateArr.length) {
        mismatches.push(`${name}.length: engine=${engineArr.length}, useState=${useStateArr.length}`);
      }
    };

    // ヘルパー: フィールド比較（nullスロットを考慮）
    const compareFields = (name, engineField, useStateField) => {
      if (!engineField && !useStateField) return;
      if (!engineField || !useStateField) {
        mismatches.push(`${name}: engine=${engineField ? 'exists' : 'null'}, useState=${useStateField ? 'exists' : 'null'}`);
        return;
      }
      const engineCount = engineField.filter(m => m !== null).length;
      const useStateCount = useStateField.filter(m => m !== null).length;
      if (engineCount !== useStateCount) {
        mismatches.push(`${name} monster count: engine=${engineCount}, useState=${useStateCount}`);
      }
    };
    ...
    if (engineState.isFirstTurn !== isFirstTurn) mismatches.push(`isFirstTurn: engine=${engineState.isFirstTurn}, useState=${isFirstTurn}`);
    if (engineState.winner !== winner) mismatches.push(`winner: engine=${engineState.winner}, useState=${winner}`);
    // logs は頻繁に更新されるため、長さのみ比較
    if (engineState.logs?.length !== logs.length) {
      mismatches.push(`logs.length: engine=${engineState.logs?.length}, useState=${logs.length}`);
    }

    // P1状態（13個）
    if (engineState.p1?.life !== p1Life) mismatches.push(`p1Life: engine=${engineState.p1?.life}, useState=${p1Life}`);
    compareArrays('p1Deck', engineState.p1?.deck, p1Deck);
    compareArrays('p1Hand', engineState.p1?.hand, p1Hand);
    compareFields('p1Field', engineState.p1?.field, p1Field);
    compareArrays('p1Graveyard', engineState.p1?.graveyard, p1Graveyard);
    if (engineState.p1?.activeSP !== p1ActiveSP) mismatches.push(`p1ActiveSP: engine=${engineState.p1?.activeSP}, useState=${p1ActiveSP}`);
    if (engineState.p1?.restedSP !== p1RestedSP) mismatches.push(`p1RestedSP: engine=${engineState.p1?.restedSP}, useState=${p1RestedSP}`);
    // fieldCard/phaseCard は存在有無のみ比較
    const p1FieldCardExists = !!engineState.p1?.fieldCard;
    const p1FieldCardUseStateExists = !!p1FieldCard;
    if (p1FieldCardExists !== p1FieldCardUseStateExists) {
      mismatches.push(`p1FieldCard: engine=${p1FieldCardExists}, useState=${p1FieldCardUseStateExists}`);
    }
    const p1PhaseCardExists = !!engineState.p1?.phaseCard;
    const p1PhaseCardUseStateExists = !!p1PhaseCard;
    if (p1PhaseCardExists !== p1PhaseCardUseStateExists) {
      mismatches.push(`p1PhaseCard: engine=${p1PhaseCardExists}, useState=${p1PhaseCardUseStateExists}`);
    }
    // statusEffects は長さのみ比較
    if ((engineState.p1?.statusEffects?.length ?? 0) !== p1StatusEffects.length) {
      mismatches.push(`p1StatusEffects.length: engine=${engineState.p1?.statusEffects?.length ?? 0}, useState=${p1StatusEffects.length}`);
    }
    if (engineState.p1?.nextTurnSPBonus !== p1NextTurnSPBonus) {
      mismatches.push(`p1NextTurnSPBonus: engine=${engineState.p1?.nextTurnSPBonus}, useState=${p1NextTurnSPBonus}`);
    }
    if (engineState.p1?.magicBlocked !== p1MagicBlocked) {
      mismatches.push(`p1MagicBlocked: engine=${engineState.p1?.magicBlocked}, useState=${p1MagicBlocked}`);
    }
    if (engineState.p1?.spReduction !== p1SpReduction) {
      mismatches.push(`p1SpReduction: engine=${engineState.p1?.spReduction}, useState=${p1SpReduction}`);
    }

    // P2状態（13個）
    if (engineState.p2?.life !== p2Life) mismatches.push(`p2Life: engine=${engineState.p2?.life}, useState=${p2Life}`);
    compareArrays('p2Deck', engineState.p2?.deck, p2Deck);
    compareArrays('p2Hand', engineState.p2?.hand, p2Hand);
    compareFields('p2Field', engineState.p2?.field, p2Field);
    compareArrays('p2Graveyard', engineState.p2?.graveyard, p2Graveyard);
    if (engineState.p2?.activeSP !== p2ActiveSP) mismatches.push(`p2ActiveSP: engine=${engineState.p2?.activeSP}, useState=${p2ActiveSP}`);
    if (engineState.p2?.restedSP !== p2RestedSP) mismatches.push(`p2RestedSP: engine=${engineState.p2?.restedSP}, useState=${p2RestedSP}`);
    const p2FieldCardExists = !!engineState.p2?.fieldCard;
    const p2FieldCardUseStateExists = !!p2FieldCard;
    if (p2FieldCardExists !== p2FieldCardUseStateExists) {
      mismatches.push(`p2FieldCard: engine=${p2FieldCardExists}, useState=${p2FieldCardUseStateExists}`);
    }
    const p2PhaseCardExists = !!engineState.p2?.phaseCard;
    const p2PhaseCardUseStateExists = !!p2PhaseCard;
    if (p2PhaseCardExists !== p2PhaseCardUseStateExists) {
      mismatches.push(`p2PhaseCard: engine=${p2PhaseCardExists}, useState=${p2PhaseCardUseStateExists}`);
    }
    if ((engineState.p2?.statusEffects?.length ?? 0) !== p2StatusEffects.length) {
      mismatches.push(`p2StatusEffects.length: engine=${engineState.p2?.statusEffects?.length ?? 0}, useState=${p2StatusEffects.length}`);
    }
    if (engineState.p2?.nextTurnSPBonus !== p2NextTurnSPBonus) {
      mismatches.push(`p2NextTurnSPBonus: engine=${engineState.p2?.nextTurnSPBonus}, useState=${p2NextTurnSPBonus}`);
    }
    if (engineState.p2?.magicBlocked !== p2MagicBlocked) {
      mismatches.push(`p2MagicBlocked: engine=${engineState.p2?.magicBlocked}, useState=${p2MagicBlocked}`);
    }
    if (engineState.p2?.spReduction !== p2SpReduction) {
      mismatches.push(`p2SpReduction: engine=${engineState.p2?.spReduction}, useState=${p2SpReduction}`);
    }

    // ターンフラグ（1個）
    if (engineState.turnFlags?.chargeUsedThisTurn !== chargeUsedThisTurn) {
      mismatches.push(`chargeUsedThisTurn: engine=${engineState.turnFlags?.chargeUsedThisTurn}, useState=${chargeUsedThisTurn}`);
    }

    if (mismatches.length > 0) {
      console.warn('[StateSync] Mismatches detected:', mismatches);
    } else {
      console.log('[StateSync] ✓ All 33 states in sync');
    }

...
> **⚠️ 重要な設計変更 (v1.1)**
> ユーザーフィードバックに基づき、以下の重要な修正を反映しています：
> 1. **トリガータイプの汎用化**: カテゴリ/属性特定トリガーを条件関数で対応（51種類 → 約20種類）
> 2. **メインフェイズトリガーのUI**: カード紐づけ型UI（基本技と同じエリアに表示）
> 3. **墓地UI実装**: 墓地カード選択と墓地トリガー発動機能
>
> 詳細は `trigger-revision-plan.md` を参照してください。

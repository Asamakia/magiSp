...
- **実装ファイル**:
  - `src/engine/keywordAbilities/index.js` - hasSourei(), canActivateSourei(), getSoureiEffect(), findSameNameCardInHand(), SOUREI_CARD_IDS, SOUREI_EFFECTS
  - `src/magic-spirit.jsx` - activateSourei()関数、【壮麗】発動ボタンUI、SP減少状態管理
- **実装メモ**:
  - 手札から同名カードを捨てる処理
  - 6種類の効果タイプ: negate_effect, grant_zankon, double_attack, damage_boost, sp_reduce, halve_attack
  - メインフェイズ中にモンスター選択時に発動ボタン表示
  - ターン1回制限（soureiUsedThisTurn フラグ）
...
- **対象カード例**:
  ...
  - C0000284: 白蛇の牙
- **実装ファイル**:
  - `src/engine/keywordAbilities/index.js` - hasDokushin(), shouldApplyDokushin(), DOKUSHIN_CARD_IDS
  - `src/magic-spirit.jsx` - ダイレクトアタック成功時に毒状態付与、p1StatusEffects/p2StatusEffects
- **実装メモ**:
  - ダイレクトアタック成功時に相手プレイヤーを毒状態にする
  - 既存のプレイヤー状態異常システム（poison）を使用
  - エンドフェイズに100ダメージ
...
- **対象カード例**:
  ...
  - C0000340: ヴェルゼファール
- **実装ファイル**:
  - `src/engine/keywordAbilities/index.js` - hasShinsyoku(), executeShinsyokuEffect(), SHINSYOKU_CARD_IDS
  - `src/engine/cardTriggers/waterCards.js` - 5カードの ON_END_PHASE_SELF トリガー
- **実装メモ**:
  - エンドフェイズトリガーとして実装済み
  - 相手モンスター1体選択（1体のみの場合は自動選択）
  - 攻撃力0判定と破壊処理
  - 破壊時は墓地送り
...
- **実装ファイル**:
  - `src/engine/keywordAbilities/index.js` - hasGigen(), canActivateGigen(), calculateGigenReduction(), GIGEN_CARD_IDS, GIGEN_MAX_REDUCTION
  - `src/magic-spirit.jsx` - startGigenActivation()、completeGigenSummon()、【犠現】発動ボタンUI、pendingGigenActivation状態
- **実装メモ**:
  - 手札から【犠現】カードを選択すると「【犠現】発動」ボタンが表示
  - ボタンクリックで生贄モンスター選択UI起動
  - 生贄のコスト分軽減（最大3）
  - 生贄は墓地送り、召喚先スロットを自動選択（空きスロット優先、なければ生贄スロット）
...
- [x] 【毒侵】- 6枚、状態異常付与 ✅ 2025-11-29

### Phase 2（戦闘・ターン終了系）
- [x] 【深蝕】- 5枚、エンドフェイズ攻撃力減少 ✅ 2025-11-28
- [x] 【魔障壁】- 3枚、魔法無効化 ✅ 2025-11-28
- [x] 【残魂】- 3枚、破壊時効果 ✅ 2025-11-29

### Phase 3（召喚・コスト系）
- [ ] 【変幻身】- 14枚、形態変化
- [x] 【犠現】- 2枚、生贄コスト軽減 ✅ 2025-11-29
- [x] 【魂結】- 6枚、リンク効果 ✅ 2025-11-28
- [x] 【覚醒】- 1枚、条件強化 ✅ 2025-11-28

### Phase 4（固有効果）
- [x] 【壮麗】- 12枚、同名カード捨てて効果 ✅ 2025-11-29
- [ ] 【威圧吼】- 1枚、攻撃時追加効果
- [ ] 【未来予知】- 1枚、デッキ操作

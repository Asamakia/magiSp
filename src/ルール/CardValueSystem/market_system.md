...
| 項目 | 内容 |
|------|------|
| 蓄積 | デイリーニュースの10%が毎日蓄積 |
| 上限 | なし（回帰圧力で自然に収束） |
| 回帰 | ±50%超で10%確率、±80%超で30%確率、±100%超で50%確率 |
| 対象 | 属性・カテゴリごとに独立 |

**蓄積例**:
```
Day 1: デイリー「炎属性 +30%」→ 永続に +3% 蓄積
Day 2: デイリー「炎属性 -20%」→ 永続に -2% 蓄積
累計: 炎属性の永続変動 = +1%
```
...
永続トレンド「未来属性」: +8.5%   ← 過去のニュース蓄積
週間トレンド「未来属性」: +20%
デイリー「禁忌研究ブーム」: +35%
突発イベントなし
─────────────────
合計: +63.5%
→ 19,575 × 1.635 = 32,005G
```
...
### 市場分析画面（MarketAnalysis）

ショップ画面のMSIセクション右上「詳細分析 →」ボタンから開く。

```
┌────────────────────────────────────────────────┐
│ 📊 市場分析                              ✕    │
├────────────────────────────────────────────────┤
│ [📊総合][📈永続][🔮属性][🏷️カテゴリ][⭐ティア][💰資産][📰イベント][🔍検索]│
├────────────────────────────────────────────────┤
│                                                │
│  ┌─────────────────────────────────────┐       │
│  │           11,234                    │       │
│  │    Magic Spirit Index (MSI)         │       │
│  │         [ 好況 ]                    │       │
│  │         ↑ +5.2%                     │       │
│  └─────────────────────────────────────┘       │
│                                                │
...
市場分析画面は**過去の価格履歴データ**を基に表示されます。
履歴は**対戦ごと（1戦 = ゲーム内1日）**に蓄積されるため、プレイ開始直後は多くのデータが表示されません。

| プレイ回数 | 表示状態 |
|------------|----------|
| 1〜3戦 | ほとんど「0」「---」「データなし」と表示される（**正常**） |
| 7戦 | 週間トレンドが1周、簡易的な傾向が見える |
| 10戦 | 突発イベントが発生しやすくなる（10%確率） |
| 30戦 | 意味のあるチャートが表示される（**推奨**） |

**Q: なぜ「0G」が多いのか？**

A: 価格履歴がまだ蓄積されていないためです。対戦を重ねることでデータが自動的に蓄積され、チャートや変動率が正しく表示されるようになります。

**各タブの説明**:

| タブ | 表示内容 | 用途 |
|------|----------|------|
| 📊 総合 | MSI（市場全体指数）+ 属性サマリー | 市場の全体像を把握 |
| 📈 永続 | 属性別・カテゴリ別の永続変動 | 長期トレンドの把握 |
| 🔮 属性 | 選択属性の詳細（現在価格、最高値、最安値、チャート） | 属性別トレンド分析 |
| 🏷️ カテゴリ | 変動率ランキング | 注目カテゴリの発見 |
| ⭐ ティア | S/A/B/C/D各ティアの推移 | ティア別投資判断 |
| 💰 資産 | 総資産推移（所持金+カード価値） | 資産管理・長期投資判断 |
| 📰 イベント | 過去のニュース・突発イベント履歴 | 価格変動の原因確認 |
| 🔍 検索 | カード名検索→個別価格チャート | 特定カードの売買判断 |

### 資産タブの詳細

総資産（所持金 + カード価値）の推移を追跡する機能。
...
**内訳カード**:
- 💰 所持金: 現在の所持金と全体に占める割合
- 🃏 カード価値（市場）: 市場価格での全所持カード価値
  - 基礎価格での価値も参考表示

**表示モード**（5種類）:
| モード | 内容 |
|--------|------|
| 総資産（市場） | 所持金 + カード価値（市場価格）|
| 総資産（基礎） | 所持金 + カード価値（基礎価格）|
| 所持金 | 所持金のみの推移 |
| カード価値（市場）| カード価値のみ（市場価格）|
| カード価値（基礎）| カード価値のみ（基礎価格）|

**チャート表示**:
- 過去30戦分の推移をスパークラインで表示
- 現在値、最高値、最安値、変動額を表示

**⚠️ 注意**: 価格履歴と同様に、資産履歴も対戦ごとに蓄積されます。

### カード詳細画面の売却価格表示

コレクション画面でカードを選択すると表示される詳細モーダル。
売却価格に市場変動が反映され、変動率も表示される。
...
| v1.4 | 2025-11-28 | 全8パターン実装、突発イベント25種実装、価格チャート実装 |
| v1.5 | 2025-11-28 | 市場分析画面の使い方・FAQ追加 |
| v1.6 | 2025-11-28 | 価格履歴に市場変動を反映、売却価格表示に市場変動を反映 |
| v1.7 | 2025-11-28 | 資産推移タブ追加（総資産/所持金/カード価値の推移チャート） |
| v1.8 | 2025-11-29 | 永続トレンドシステム追加（デイリー10%蓄積、回帰圧力） |
...
| 機能 | ファイル | 説明 |
|------|----------|------|
| ✅ 市場エンジン | `marketEngine.js` | 価格変動の計算ロジック |
| ✅ 週間トレンド（15種） | `weeklyTrend.js` | 7戦ごとに切り替わるトレンド |
| ✅ デイリーニュース（8パターン） | `newsGenerator.js` | テンプレート自動生成システム |
| ✅ 突発イベント（25種） | `suddenEvents.js` | 高騰10種・暴落8種・特殊7種 |
| ✅ 価格履歴 | `priceHistory.js` | 30戦分の履歴保持、市場変動込みで記録 |
| ✅ 価格チャートUI | `PriceChart.jsx` | スパークライン、市場指数表示 |
| ✅ ショップUI統合 | `ShopScreen.jsx` | ニュース・トレンド・突発イベント表示 |
| ✅ 売却価格表示 | `CardDetail.jsx` | 市場変動込みの売却価格・変動率表示 |
| ✅ 売買処理 | `shopSystem.js` | 市場価格での売買処理 |

### データファイル

| ファイル | 内容 |
|----------|------|
| `categories.js` | カテゴリ→属性マッピング（21カテゴリ） |
| `reasons.js` | 理由（30種） |
| `persons.js` | 人物（15種）と行動傾向 |
| `locations.js` | 場所（15種）と関連出来事 |
| `rumors.js` | 噂（9種）と関連対象 |
| `comparisons.js` | 比較ペア（14組） |
| `supplyDemand.js` | 需給対象（13種） |
| `seasonal.js` | 季節/時間（8種）と関連属性 |
| `characters.js` | キャラ/勢力（15種）と関連対象 |

### 定数設定

| 定数 | 値 | 説明 |
|------|-----|------|
| `DAYS_PER_WEEK` | 7 | 週間トレンド切替間隔 |
| `SUDDEN_EVENT_CHANCE` | 10% | 突発イベント発生確率 |
| `MAX_MODIFIER_UP` | +100% | 上昇上限（最大2倍） |
| `MAX_MODIFIER_DOWN` | -50% | 下落下限（最低半額） |
| `HISTORY_LENGTH` | 30 | 価格履歴保持戦数 |
| `PERSISTENT_ACCUMULATION_RATE` | 10% | デイリー→永続変動の蓄積率 |
| `REGRESSION_THRESHOLDS` | 50/80/100% | 回帰圧力発動閾値 |
...
```
保存: 市場価格（基礎価値 × 市場変動）の推移
表示: レアリティ価格は倍率をかけて都度計算

例: フレア・ドラゴンの履歴
  保存 → [360, 396, 324, 370, ...]（市場変動込み30戦分）
         （ニュースで+10%→396, -10%→324 など）
  表示 → UR価格 = 保存値 × 15 で計算
```

**重要**: v1.6以前は基礎価値のみ保存していたため、属性別サマリーの変動率が常に0%でした。
現在は市場変動を反映した価格を記録するため、ニュースやトレンドに応じて変動率が変化します。
...
const priceHistory = {
  // 個別カード（市場変動込みの価格を保存）
  cards: {
    'MS-F-001': [360, 396, 324, 370, ...], // 30戦分（市場変動込み）
    'MS-F-002': [250, 275, 237, 270, ...],
    // ... 433カード分
  },

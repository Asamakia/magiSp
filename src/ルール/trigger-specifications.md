...
> **⚠️ 重要な設計変更 (v1.1)**
> 1. **トリガータイプの汎用化**:
>    - カテゴリ/属性特定トリガー（【ドラゴン破壊時】など）は削除
>    - 汎用トリガー + 条件関数で実装
>    - 例: `ON_CATEGORY_MONSTER_DESTROYED` + `condition: (ctx) => ctx.card.category.includes('【ドラゴン】')`
>
> 2. **メインフェイズトリガーは任意発動**:
>    - 【自分メインフェイズ時】は基本技と同じUIエリアに表示
>    - カード選択時に発動ボタンを表示（カード紐づけ型）
>    - フェーズ開始時の自動発火ではない
>
> 3. **発動タイプの追加**:
>    - `AUTOMATIC`（自動・強制発動）- デフォルト
>    - `OPTIONAL`（任意発動）- カード固有で指定
>
...
- **発動条件**: **このカード自身**が手札からフィールドに配置された時
- **効果範囲**: デッキ検索、墓地蘇生、ダメージ、ドローなど多様
- **実装箇所**: `summonCard()` 関数内
- **注意点**:
  - 召喚コスト支払い**後**に発動
  - **自分自身が召喚された時のみ発火**（他のモンスターが召喚されても発火しない）
  - 他のモンスターの召喚に反応する場合は `ON_OPPONENT_SUMMON` 等の別トリガーを使用
